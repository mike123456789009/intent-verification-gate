...
Any packet that asks the reviewer to compare or grade main-agent output must
include this source disclosure: "The main-agent output you are grading was
generated by a Claude model." This disclosure is provenance metadata only and
does not change the scoring rubric.
...
## Rerunning a past gate

Use the `rerun` command when an existing run needs a fresh review round without
losing prior review history.

```bash
python3 "$IVG" rerun --run-dir /abs/path/to/run --json
```

When the exact run path is not known, a model may select from repo history:
...
- validates the selected run first and records its pre-rerun status
- allocates the next `review-0N.md` file
- keeps every previous `review-0N.md` unchanged
- updates `gate-status.json` to `reviewing`
- returns exact `review-packet` commands pinned to the new review file
- returns a validation command for the model to run after the new review is filled

Do not overwrite an old review file for a rerun. If `--match` finds multiple
runs, use `--run-dir` for the exact run or `--latest` to intentionally select
the newest match.
